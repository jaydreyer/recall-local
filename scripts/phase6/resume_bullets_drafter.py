#!/usr/bin/env python3
"""Generate Phase 6 resume-bullet artifacts from resume and evaluated jobs."""
...
def _resume_bullets_prompt(*, job: dict[str, Any], resume_text: str) -> str:
    ...
    if isinstance(gaps, list):
        for item in gaps[:4]:
            if isinstance(item, str) and item.strip():
                gap_names.append(item.strip())
            elif isinstance(item, dict):
                name = str(item.get("gap") or item.get("skill") or item.get("name") or "").strip()
                if name:
                    gap_names.append(name)

    skills_text = ", ".join(matching_skill_names) if matching_skill_names else "Not provided"
    gaps_text = ", ".join(gap_names) if gap_names else "No major gaps provided"
    ...
    return (
        "Write concise tailored resume bullets in plain text.\n"
        "Requirements:\n"
        ...
        f"Gaps to avoid overstating: {gaps_text}\n"
...
def _save_to_vault(*, job: dict[str, Any], bullets_text: str) -> str | None:
    ...
    output_path = folder / "resume-bullets.md"
    output_path.write_text(
        "\n".join(
            [
                f"# Resume Bullets - {job.get('company')} - {job.get('title')}",
                "",
                f"Generated: {now_iso()}",
...
def generate_resume_bullets(
    *,
    ...
    prompt = _resume_bullets_prompt(job=job, resume_text=resume_text)
    ...
    return {
        "resume_bullets_id": f"resume_bullets_{_slugify(normalized_job_id)}",
        "job_id": normalized_job_id,
        "provider": provider,
        "model": model,
        "generated_at": now_iso(),
        "bullet_count": len([line for line in cleaned_bullets.splitlines() if line.strip().startswith("- ")]),
        "word_count": len([word for word in cleaned_bullets.split() if word and word != "-"]),
