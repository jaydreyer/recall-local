#!/usr/bin/env python3
"""Generate Phase 6 tailored summary artifacts from resume and evaluated jobs."""
...
def _summary_prompt(*, job: dict[str, Any], resume_text: str) -> str:
    ...
    return (
        "Write a concise tailored resume summary in plain text.\n"
        "Requirements:\n"
...
def _save_to_vault(*, job: dict[str, Any], summary_text: str) -> str | None:
    ...
    output_path = folder / "tailored-summary.md"
    output_path.write_text(
        "\n".join(
            [
                f"# Tailored Summary - {job.get('company')} - {job.get('title')}",
                "",
                f"Generated: {now_iso()}",
...
def generate_tailored_summary(
    *,
    ...
    prompt = _summary_prompt(job=job, resume_text=resume_text)
    ...
    return {
        "summary_id": f"tailored_summary_{_slugify(normalized_job_id)}",
        "job_id": normalized_job_id,
        "provider": provider,
        "model": model,
        "generated_at": now_iso(),
        "word_count": len([word for word in cleaned_summary.split() if word and word != "-"]),
