#!/usr/bin/env python3
"""Integration-style pytest coverage for the Phase 1 -> Phase 6 cover-letter flow."""
...
def test_cover_letter_endpoint_uses_phase6_drafter_dependencies(monkeypatch) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cover_letter_drafter, "_load_resume_text", lambda: "Resume context here.")
    monkeypatch.setattr(
        cover_letter_drafter,
    ...
    monkeypatch.setattr(
        cover_letter_drafter,
        "_call_ollama",
        lambda prompt, settings: "Dear Hiring Team,\n\nI build AI platform workflows.",
    )
    ...
    body = response.json()
    assert body["workflow"] == "workflow_06a_cover_letter_draft"
    assert body["job_id"] == "job-1"
    assert body["provider"] == "ollama"
    assert body["model"] == "qwen2.5:7b-instruct"
    assert "I build AI platform workflows." in body["draft"]
