#!/usr/bin/env python3
"""Integration-style pytest coverage for the Phase 1 -> Phase 6 resume-bullets flow."""
...
def test_resume_bullets_endpoint_uses_phase6_drafter_dependencies(monkeypatch) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(resume_bullets_drafter, "_load_resume_text", lambda: "Resume context here.")
    monkeypatch.setattr(
        resume_bullets_drafter,
    ...
    body = response.json()
    assert body["workflow"] == "workflow_06a_resume_bullets"
    assert body["job_id"] == "job-1"
    assert body["provider"] == "ollama"
    assert body["model"] == "qwen2.5:7b-instruct"
    assert "Built AI platform workflows." in body["bullets"]
