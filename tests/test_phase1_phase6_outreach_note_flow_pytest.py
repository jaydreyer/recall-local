#!/usr/bin/env python3
"""Integration-style pytest coverage for the Phase 1 -> Phase 6 outreach-note flow."""
...
def test_outreach_note_endpoint_uses_phase6_drafter_dependencies(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda job_id: {
            ...
            "cover_letter_angle": "Lead with platform empathy and customer-facing delivery.",
        },
    )
    monkeypatch.setattr(outreach_note_drafter, "_load_resume_text", lambda: "Resume context here.")
    monkeypatch.setattr(
        outreach_note_drafter,
    ...
    monkeypatch.setattr(
        outreach_note_drafter,
        "_call_ollama",
        lambda prompt, settings: "Hi team,\n\nI’m reaching out because this role is a close match for my customer-facing AI workflow and API rollout background.\n\nBest,\nJay",
    )
    ...
    body = response.json()
    assert body["workflow"] == "workflow_06a_outreach_note"
    assert body["job_id"] == "job-1"
    assert body["provider"] == "ollama"
    assert body["model"] == "qwen2.5:7b-instruct"
    assert "customer-facing AI workflow" in body["note"]
