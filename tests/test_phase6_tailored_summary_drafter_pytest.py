#!/usr/bin/env python3
"""Pytest coverage for Phase 6 tailored summary helpers."""
...
def test_clean_summary_normalizes_bullets(raw_text: str, expected: str) -> None:
    assert tailored_summary_drafter._clean_summary(raw_text) == expected


def test_generate_tailored_summary_local_mode_returns_cleaned_text(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(tailored_summary_drafter, "_load_resume_text", lambda: "Resume with API platform work.")
    monkeypatch.setattr(
        tailored_summary_drafter,
    ...
    assert result["summary"].startswith("- Built AI workflow systems.")
    assert result["word_count"] > 0
    assert result["saved_to_vault"] is False


def test_generate_tailored_summary_can_write_to_vault(
    ...
    monkeypatch.setattr(tailored_summary_drafter, "_load_resume_text", lambda: "Resume text.")
    monkeypatch.setattr(
        tailored_summary_drafter,
        "_load_runtime_settings",
